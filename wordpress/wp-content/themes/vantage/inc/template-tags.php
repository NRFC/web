			...
			<div class="box-content entry-content">
				<div class="box-title">
					...
					<span class="author-posts">
						<a href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ) ); ?>">
							<?php
							esc_html_e( 'View posts by ', 'vantage' );
							echo get_the_author();
							?>
						</a>
					</span>
...
if ( ! function_exists( 'vantage_content_nav' ) ) {
	...
	function vantage_content_nav( $nav_id ) {
		...
		global $wp_query, $person;

		// Don't print empty markup on single pages if there's nowhere to navigate.
		if ( is_single() ) {
			$previous = ( is_attachment() ) ? get_post( $person->post_parent ) : get_adjacent_post( false, '', true );
...
} // ends check for vantage_comment()
